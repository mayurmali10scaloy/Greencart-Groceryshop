...
function EditProduct() {
  ...
  useEffect(() => {
    ...
    api.get(`/products/${id}`)
      .then((res) => {
        console.log("Fetched product response data:", res.data);
        ...
        setProduct({
          ...
          ratings: productData.ratings ?? "",  // ✅ pre-populate rating
          image: productData.productImg ? `http://localhost:5005/uploads/products/${productData.productImg}` : "",
          isActive: productData.isActive ?? true, // ✅ pre-populate isActive
        });
        if (productData.productImg) {
          setFileName(productData.productImg);
        }
      })
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      // Navigate back with state for notification
      nav("/AdminPanel/products", {
        state: { msg: "Product Updated Successfully!", type: "edit" },
      });
    } catch (err) {
  ...
  const finalPrice = product.price
    ? Math.round(product.price - (product.price * (product.discountPercent || 0)) / 100)
    : 0;
  const savedAmount = product.price && product.discountPercent
    ? Math.round((product.price * product.discountPercent) / 100)
    : 0;
  return (
        ...
        <div className="modal-header-bar">
          <div className="modal-header-left">
            ...
            <div>
              ...
              <p className="modal-subtitle">Modify the details of your existing product</p>
            </div>
          </div>
            ...
            <div className="modal-col-1">
              ...
              <div className="form-grid single">
                ...
                <div className="field-group">
                  ...
                  <input type="text" className="field-input" placeholder="Enter brand"
        ...
        <div className="modal-footer-bar">
          ...
          <div className="modal-footer-actions">
            ...
            <button type="submit" className="btn-save" disabled={converting}>
              {converting ? <><i className="fa-solid fa-spinner fa-spin"></i> Processing...</> : "Update Product"}
            </button>
          </div>
        </div>
